...
import com.example.elasticjob.service.bo.JobPacketBO;
...
public class ElasticJobServiceImpl implements ElasticJobService {
    ...
    @Override
    public void update(String jobName, JobPacketBO jobPacket) {
        JobSettingsAPI jobSettingAPI = getJobSettingAPI();
        JobSettings jobSettings = jobSettingAPI.getJobSettings(jobPacket.getJobName());
        jobSettings.setJobName(jobPacket.getJobName());
        jobSettings.setCron(jobPacket.getCron());
        jobSettings.setShardingTotalCount(jobPacket.getShardingTotalCount());
        jobSettings.setShardingItemParameters(jobPacket.getShardingItemParameters());
        jobSettings.setJobParameter(jobPacket.getJobParameter());
        jobSettings.setFailover(jobPacket.isFailover());
        jobSettings.setMisfire(jobPacket.isMisfire());
        jobSettings.setDescription(jobPacket.getDescription());
        jobSettings.setJobProperties(jobPacket.getJobProperties());
        jobSettings.setStreamingProcess(jobPacket.isStreamingProcess());
        jobSettingAPI.updateJobSettings(jobSettings);
    }
    ...
    private JobSettingsAPI getJobSettingAPI() {
        if (jobSettingsAPI == null) {
            jobSettingsAPI = JobAPIFactory.createJobSettingsAPI(zookeeperConfiguration.getServerLists(), zookeeperConfiguration.getNamespace(),
                    Optional.fromNullable(zookeeperConfiguration.getDigest()));
        }
